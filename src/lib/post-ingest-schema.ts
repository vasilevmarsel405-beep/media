...
export const makeIngestPostSchema = z
  .object({
    ...
    homePick: z.boolean().optional(),
    homeMain: z.boolean().optional(),
    homeProject: z.boolean().optional(),
    homeVideoUrl: homeVideoUrlIngestZ,
    homeVideoLabel: z.string().max(120).optional(),
    seoTitle: z.string().min(1).max(80).optional(),
    seoDescription: z.string().min(1).max(350).optional(),
...
export function normalizeIngestToPost(input: MakeIngestPostInput): import("./types").Post {
  ...
  const base = {
    ...
    ...(input.imageAlt !== undefined ? { imageAlt: sanitizeIngestText(input.imageAlt) } : {}),
  ...
  const optional = {
    ...
    ...(input.homePick !== undefined ? { homePick: input.homePick } : {}),
    ...(input.homeMain !== undefined ? { homeMain: input.homeMain } : {}),
    ...(input.homeProject !== undefined ? { homeProject: input.homeProject } : {}),
