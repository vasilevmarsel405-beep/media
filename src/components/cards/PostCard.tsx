      ...
      <article className="card-hover group flex gap-4 rounded-2xl border border-slate-100 bg-white p-3 shadow-sm">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
            {post.urgent ? (
              <span className="rounded bg-red-600 px-2 py-0.5 font-bold uppercase tracking-wide text-white">Срочно</span>
            ) : null}
            {primaryRubric ? (
              <Link href={`/rubriki/${primaryRubric.slug}`} className="font-semibold text-sky-700 hover:underline">
                {primaryRubric.name}
              </Link>
            ) : null}
    ...
    return (
      <article className="border-b border-slate-100 py-4 last:border-0">
        <div className="flex items-start justify-between gap-3">
          <div>
            {post.urgent ? (
              <span className="text-xs font-bold uppercase text-red-600">Срочно · </span>
            ) : null}
            <Link href={href} className="font-display text-base font-semibold text-slate-900 hover:text-slate-700">
              {post.title}
            </Link>
            <p className="mt-1 text-sm text-slate-600">{post.lead}</p>
          </div>
          <time className="shrink-0 text-xs text-slate-400" dateTime={post.publishedAt}>
