...
- Ответ `{"ok":true,"saved":true,...}` означает: запись в Redis уже выполнена и сброшен in-memory кеш ленты; инвалидация путей Next.js (`revalidatePath`) ставится в очередь сразу после ответа, чтобы не блокировать сайт и не ловить **504** у всех посетителей.
- Если nginx снова отдаёт **504**, увеличьте в `location` прокси к Node таймауты (`proxy_read_timeout` и т.д.) — см. `docs/nginx-cryptomarsmedia.conf.example`.
- По умолчанию webhook в **лёгком** режиме: главная + список по виду + страница материала. Полный проход по всем путям — **`MAKE_WEBHOOK_LIGHT_REVALIDATE=0`** (тяжелее для VPS).
